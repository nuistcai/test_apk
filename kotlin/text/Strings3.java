package kotlin.text;

import kotlin.Metadata;
import kotlin.Tuples;
import kotlin.Tuples3;
import kotlin.jvm.functions.Function2;
import kotlin.jvm.internal.Intrinsics;
import kotlin.jvm.internal.Lambda;

/* compiled from: Strings.kt */
@Metadata(bv = {1, 0, 3}, d1 = {"\u0000\u0012\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\b\n\u0002\u0010\r\n\u0002\b\u0002\u0010\u0000\u001a\u0010\u0012\u0004\u0012\u00020\u0002\u0012\u0004\u0012\u00020\u0002\u0018\u00010\u0001*\u00020\u00032\u0006\u0010\u0004\u001a\u00020\u0002H\n¢\u0006\u0002\b\u0005"}, d2 = {"<anonymous>", "Lkotlin/Pair;", "", "", "currentIndex", "invoke"}, k = 3, mv = {1, 1, 15})
/* renamed from: kotlin.text.StringsKt__StringsKt$rangesDelimitedBy$2 */
/* loaded from: classes.dex */
final class Strings3 extends Lambda implements Function2<CharSequence, Integer, Tuples<? extends Integer, ? extends Integer>> {
    final /* synthetic */ char[] $delimiters;
    final /* synthetic */ boolean $ignoreCase;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    Strings3(char[] cArr, boolean z) {
        super(2);
        delimiters = cArr;
        ignoreCase = z;
    }

    @Override // kotlin.jvm.functions.Function2
    public /* bridge */ /* synthetic */ Tuples<? extends Integer, ? extends Integer> invoke(CharSequence charSequence, Integer num) {
        return invoke(charSequence, num.intValue());
    }

    public final Tuples<Integer, Integer> invoke(CharSequence receiver, int currentIndex) {
        Intrinsics.checkParameterIsNotNull(receiver, "$receiver");
        int it = StringsKt.indexOfAny(receiver, delimiters, currentIndex, ignoreCase);
        if (it < 0) {
            return null;
        }
        return Tuples3.to(Integer.valueOf(it), 1);
    }
}